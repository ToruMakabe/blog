...
なおSecretは実行サーバー上で永続化されず、tmpfs(揮発ファイルシステム)に書き込まれます。
...
値はBase64で符号化した上で書きます。当然ながらBase64でデコードできるので、マニフェストの管理はご注意を。
...
これでOKです。アプリケーションは環境変数を読むのに変わりがないため、そのままで構いません。なお、SecretはBase64で管理されていますが、アプリケーションにはデコードして送られます。

もちろんPodの参照権限だけでは、中身をそのまま見ることはできません。
...
また、マニフェストに値を書かないため、シークレットの変更時、影響を受けるマニフェストを変更する必要がないのもメリットです。Secretを更新すれば済みます。
...
確認される環境変数名など、詳細は[公式ドキュメント](https://docs.microsoft.com/ja-jp/go/azure/azure-sdk-go-authorization#use-environment-based-authentication)をご確認を。

このサンプルでは、クライアント資格情報、つまりサービスプリンシパルを使ってKey Vaultの認証を行います。環境変数 AZURE_TENANT_ID、AZURE_CLIENT_ID、AZURE_CLIENT_SECRET が設定された環境で auth.NewAuthorizerFromEnvironment() を呼ぶと、クライアント資格情報を使った認証が選択されます。

### Key Vault名、シークレット名、バージョン名を環境変数で渡す

これは見ての通りですね。シークレットが格納されているKey Vaultを指定するため、 AZURE_KEYVAULT_NAME などを環境変数から読み込みます。

他言語でどう書くかは、それぞれSDKのドキュメントを参考にしてください。
...
Key VaultをVolumeとしてマウントしていることが、分かるでしょうか。
...
なお、HashiCorpのVaultも、似たようなコンセプトです。
