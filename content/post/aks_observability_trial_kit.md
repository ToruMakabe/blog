...
ノードの平均CPU、メモリ利用率、ノードの状態やポッドの状態が対象です。種類は多くありませんが、クリティカルなメトリックを取得できます。
...
メトリクスバックエンドとしてPrometeus、メトリクス可視化としてGrafanaを導入し、Kubernetes向けの基本的な設定は済ませています。まずは分かりやすく、Grafanaでメトリクスを見てみましょう。
...
すると、以下のルール KubePodCrashLooping にマッチし、Pending状態になります。
